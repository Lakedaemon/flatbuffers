...
class GeneralGenerator : public BaseGenerator {
  ...
  bool generate() {
    ...
    for (auto it = parser_.enums_.vec.begin(); it != parser_.enums_.vec.end();
      ...
      if (parser_.opts.one_file) {
        ...
        if (!SaveType((**it).name, enumcode, false)) return false;
      }
    }
    ...
    for (auto it = parser_.structs_.vec.begin();
      ...
      if (parser_.opts.one_file) {
        ...
        if (!SaveType((**it).name, declcode, true)) return false;
      }
    }

    if (parser_.opts.one_file) {
      return SaveType(file_name_, one_file_code, true);
    }
    return true;
  }
 ...
 private:
  ...
  bool SaveType(const std::string &defname, const std::string &classcode,
                bool needs_includes) {
    ...
    auto filename = namespace_dir_ + defname + lang_.file_extension;
...
#undef FLATBUFFERS_TD
  ...
  // For example, a long holding an unsigned int value would be cast down to int
  // before being put onto the buffer.
  // In C#, one cast directly cast an Enum to its underlying type, which is
  // essential before putting it onto the buffer.
  ...
  void GenStruct(StructDef &struct_def, std::string *code_ptr) {
    ...
    for (auto it = struct_def.fields.vec.begin();
      ...
      if (lang_.language == IDLOptions::kCSharp &&
        ...
        // For scalars, default value will be returned by GetDefaultValue(). If
        // the scalar is an enum, GetDefaultValue()
        // returns an actual c# enum that doesn't need to be casted. However,
        // default values for enum elements of
      ...
      if (IsScalar(field.value.type.base_type)) {
        ...
        switch (field.value.type.base_type) {
          ...
          case BASE_TYPE_VECTOR: {
            ...
            auto index =
                "__vector(o) + j * " + NumToString(InlineSize(vectortype));
            if (vectortype.base_type == BASE_TYPE_STRUCT) {
              code += vectortype.struct_def->fixed ? index : "__indirect(" +
                                                                 index + ")";
              code += ", bb";
            } else {
              code += index;
    ...
    if (struct_def.fixed) {
      ...
      if (parser_.root_struct_def_ == &struct_def) {
        ...
        code += "Buffer(FlatBufferBuilder builder, " +
                GenOffsetType(struct_def) + " offset) {";
